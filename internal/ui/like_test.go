...
// ---------------------------------------------------------------------------
// DIMM-382: dislike (`d`) toggle
// ---------------------------------------------------------------------------

// TestDislike_noopWhenNoTrack mirrors the like equivalent: `d` is silent
...
func TestDislike_noopWhenNoTrack(t *testing.T) {
	...
	if cmd != nil {
		t.Errorf("expected nil Cmd on `d` with no track")
	}
...
// TestDislike_firstPressDislikes verifies pressing `d` on a neutral track
...
// TestDislike_secondPressClears verifies pressing `d` on an already-
...
// TestDislike_onLikedClearsLike verifies that pressing `d` on a liked
