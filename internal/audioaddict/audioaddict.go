...
type authPayload struct {
	...
	AudioToken string `json:"audio_token"` // top-level, NOT inside member
	Member     struct {
		ID            int64  `json:"id"`
...
// SkipResponse is the server's answer to a POST /skip_events request.
// The caller uses SkipsRemaining to show a toast ("3 skips left").
type SkipResponse struct {
	SkipsRemaining int    `json:"skips_remaining"`
	Expires        string `json:"expires"`
}

// skipEventBody is the JSON body sent to POST /skip_events. Field names
// and null-valued fields match the web player's Backbone model exactly
// (inspected from di.fm's minified JS bundle — see DIMM-408).
type skipEventBody struct {
	TrackID        int64   `json:"track_id"`
	ChannelID      int64   `json:"channel_id"`
	SkippedAt      int     `json:"skipped_at"`
	Length         int     `json:"length"`
...
// SkipTrack posts a skip event for the currently-playing track, causing
// the server to record the skip (rate-limited per tier). trackLength is
// the track duration in seconds (from Track.Duration); skippedAt is how
// many seconds into the track the user pressed skip (0 is fine). The
// caller should re-tune the channel after a successful skip so the
// stream advances.
//
//	POST /v1/<network>/skip_events
func (c *Client) SkipTrack(ctx context.Context, network string, trackID, channelID int64, trackLength, skippedAt int) (*SkipResponse, error) {
	sess := c.currentSession()
	if sess.SessionKey == "" {
		dlogf(c.Debug, "SkipTrack pre-flight: SessionKey empty — returning ErrSessionInvalid")
		return nil, ErrSessionInvalid
	}

	// moment().toString() format: "Sun Jul 06 2026 18:44:26 GMT+0000"
	now := time.Now().UTC()
	createdAt := now.Format("Mon Jan 02 2006 15:04:05 GMT-0700")
	payload := skipEventBody{
		TrackID:   trackID,
		ChannelID: channelID,
		SkippedAt: skippedAt,
		Length:    trackLength,
	...
	if err != nil {
		return nil, fmt.Errorf("skip_events: encode: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost,
		c.BaseURL+"/"+network+"/skip_events", strings.NewReader(string(jsonBody)))
	if err != nil {
		return nil, err
	}
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-Session-Key", sess.SessionKey)
	req.Header.Set("X-MP-Ruleset", "true")

	dlogf(c.Debug, "SkipTrack -> POST %s/%s/skip_events body=%s",
		c.BaseURL, network, truncate(string(jsonBody), 300))
	...
	defer func() { _ = resp.Body.Close() }()
	body, _ := io.ReadAll(resp.Body)
	dlogf(c.Debug, "SkipTrack <- status=%d body=%q", resp.StatusCode, truncate(string(body), 400))

	switch resp.StatusCode {
	case http.StatusOK, http.StatusCreated:
		var sr SkipResponse
		if err := json.Unmarshal(body, &sr); err != nil {
			return nil, fmt.Errorf("skip_events: decode: %w", err)
		}
		return &sr, nil
	case http.StatusUnauthorized, http.StatusForbidden:
		return nil, ErrSessionInvalid
	...
	default:
		return nil, fmt.Errorf("POST /%s/skip_events: unexpected status %d: %s",
			network, resp.StatusCode, truncate(string(body), 200))
	}
}
